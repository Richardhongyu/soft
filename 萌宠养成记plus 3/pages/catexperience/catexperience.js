...
Page({
  ...
  getDataFromOneNet: function () {
    ...
    const requestTask = wx.request({
      ...
      success: function (res) {
        ...
        page.humlSet(huml);
        //给四个变量从onenet上面赋值，并且调用函数，让他们对经验值进行改变
  ...
  //下面的4个函数可以对经验值进行改变
  // 改变的规则是：温度在20-30间，经验值+1；温度在30-35之间时，经验值不变；不在这几个范围的时候，经验值不变
  //设置温度
  ...
  // 改变的规则是：光强在300-600间，经验值+1；光强在200-300经验值不变；不在这几个范围的时候，经验值-1
  //设置光强
  ...
  // 改变的规则是：湿度在20-30间，经验值不变；温度在30-35之间时，经验值-2；不在这几个范围的时候，经验值不变
  //设置湿度
  ...
  //下面这个函数是对经验值与等级进行处理的函数，并且进行数据绑定，然后输出 
  level_: function (level, expe) {
    ...
    //对exper，levelq，percentNum进行数据绑定，以便在wxml里面输出
  },
})
